...
export function timeline(cv, nowYear) {
  ...
  const renderTrack = (entries, track) => {
    ...
    const bars = entries.map((e, i) => {
      ...
      // A real anchor, not a click handler: it navigates without JS, takes
      // keyboard focus, and offers "open in new tab" like any other link. The
      // detail entry carries the matching id.
      const k = keyOf(e);
