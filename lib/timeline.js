...
function laneSegments(entries, now) {
  ...
  // Parked buses. Each bar takes one bay for its whole run and holds it —
  // straight, never stepping sideways — and each takes the innermost bay it
  // can, so the rail stays packed toward the spine.
  //
  // Buses arrive in the order the detail column lists them, NOT in date order.
  // That is the whole point: the reader meets these roles in a particular
  // sequence on the left, and the rail should fill in that same sequence. Under
  // date order, Code Chrysalis took the innermost bay merely because one had
  // fallen vacant by 2019, despite being the last entry in the list.
  //
  // Because arrivals are no longer chronological, a bay cannot be checked with
  // "has the last bus left yet" — that only holds when buses arrive in time
  // order. Each bay keeps its parked intervals and a candidate must clear all
  // of them.
  const bays = [];                       // bays[n] = intervals parked in bay n
  const lanes = new Array(entries.length).fill(0);
  const free = (bay, bus) => bay.every((p) => bus.to <= p.from || bus.from >= p.to);

  for (const bus of iv) {                // iv is already in content order
    let n = bays.findIndex((bay) => free(bay, bus));
    if (n === -1) { n = bays.length; bays.push([]); }
    bays[n].push(bus);
    lanes[bus.i] = n;
  }
  const vacatedAt = bays;                // named for the return below
